...
class CabFacturasModel {
    ...
    public function actualizarCostoTotalFactura($COD_CAB_FACT, $COSTO_TOT_CAB_FACT) {
        $pdo = Database::connect();
        $sql = 'update tab_fac_cab_facturas set COSTO_TOT_CAB_FACT=? where COD_CAB_FACT=?';
        $consulta = $pdo->prepare($sql);
        try {
            $consulta->execute(array($COSTO_TOT_CAB_FACT, $COD_CAB_FACT));
        } catch (PDOException $e) {
            Database::disconnect();
            throw new Exception($e->getMessage());
        }
    ...
    // MÉTODO PARA FILTRAR FACTURAS POR FECHAS
    public function getFiltradoFacturasFecha($fecha_inicio, $fecha_fin) {
    ...
    public function getFiltradoFacturasMensual($mes_inicio, $mes_fin) {
        //obtenemos la informacion de la bdd:
        $pdo = Database::connect();
